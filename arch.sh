...
verificar_nao_root() {
    if [ "$EUID" -eq 0 ]; then
        ...
        echo -e "O script pedirá sua senha automaticamente (via sudo) quando for necessário.\n"
        exit 1
    fi
}
...
verificar_yay() {
    echo -e "\n${AZUL}==================================================${NC}"
    echo -e "${VERDE}        VERIFICANDO GERENCIADOR DE PACOTES        ${NC}"
    echo -e "${AZUL}==================================================${NC}"
    
    if ! command -v yay &> /dev/null; then
        echo -e "${AMARELO}[!] O 'yay' (AUR Helper) não foi encontrado.${NC}"
        echo -e "[*] Instalando dependências base e o yay..."
        ...
        echo -e "${VERDE}[✔] 'yay' instalado com sucesso!${NC}"
    else
        echo -e "${VERDE}[✔] 'yay' já está instalado no sistema.${NC}"
    fi
}
...
instalar_grupo() {
    ...
    if yay -S --needed --noconfirm "${PACOTES[@]}" > /dev/null 2>&1; then
...
instalar_pacotes() {
    echo -e "\n${AZUL}==================================================${NC}"
    echo -e "${VERDE}      INICIANDO INSTALAÇÃO DE PACOTES VIA YAY     ${NC}"
    echo -e "${AZUL}==================================================${NC}"
    ...
    APPS_MULTIMIDIA=(
        "strawberry" "gst-plugins-bad" "gst-plugins-ugly" "gst-libav"
    ...
    if [ "$INSTALAR_PYTHON" == "s" ]; then
        ...
        PYTHON_LIBS_NATIVAS=(
            ...
            "python-pyqt5" "pyside6" "python-kivy" "wxpython"
...
configurar_ambiente_python() {
    ...
    echo "Instalando ${#PIP_PKGS[@]} bibliotecas Python adicionais no VENV..."
...
gerar_relatorio() {
    ...
    echo "# 📋 Resumo da Instalação - Arch Linux Autotools" > "$RELATORIO"
